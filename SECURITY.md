...
This repository is the **Android client SDK** — code that runs on end users' personal devices,
inside our customers' apps. Findings we especially want to hear about:
...
- **Device-identity problems.** The device ID is a pseudonymous identifier under GDPR (Founding
  ...
  from, or reversible to, `ANDROID_ID`, IMEI, a serial number, an advertising ID or end-user PII
  is a privacy finding, not a design preference.
...
- **We cannot recall a shipped SDK.** A vulnerable version lives in customers' apps until they
  rebuild and ship, on their schedule and their app store's. That makes findings here longer-lived
  than anywhere else in the system, and worth reporting even when they look minor.
...
`FortressFlag_SDK_ios` (iOS client SDK), `FortressFlag_Standards` (contracts).
